/*
 * TestZ80.java - Minimal Z80 processor interface for testbench.
 * 
...
public interface TestZ80 {
     ...
     *
     * 0: IFF1 (interrupts enabled ?) 1: IFF2 (backup of IFF1 in NMI) 2-3:
     ...
     * <code>10h-20h=-10h=F0h</code> or
     * <code>c0h+55h=15h</code> do.
     */
    public static final int CF = 0x01;
    /**
     * Add-subtract flag. Usually set when addition and increasing commands are
     * performed and reset when subtraction, decreasing or comparison commands
     * are performed.
     */
    public static final int NF = 0x02;
    /**
     * Parity-oVerflow-flag. In addition and subtraction this is set in signed
     * overflows, for example when two positive (7-bit 0) numbers are added to
     * produce a negative (7-bit 1) number. In logical commands this is set if
     * the number of 1-bits in the result is even.
     */
    ...
    /**
     * This flag is set if there is an unsigned overflow from 3-bit in 8-bit
     * arithmetic operations and from 11-bit in 16-bit arithmetic. It is used
     * only in DAA command when decimal adjusting results of additions and
     * subtractions.
     */
    ...
    /**
     * This flag is set if the previous arithmetic or logical operation results
     * in zero or if a comparison by CP, CPI or CPIR succeeds.
     */
    public static final int ZF = 0x40;
    /**
     * An <i>official</i> flag usually holding a copy of 7-bit of the result,
     * which conforms to the sign bit in two's complement system. Used mostly in
     * <code>JP PO</code>,
     * <code>JP PE</code> etc. commands.
     */
    ...
    /**
     * Gets value of a register pair. Note that AF and AF_ALT register pairs are
     * special, as their register indices are big-endian while other register
     * pairs are little-endian.
     *
     * @param regpairno	Number of register pair affected.
