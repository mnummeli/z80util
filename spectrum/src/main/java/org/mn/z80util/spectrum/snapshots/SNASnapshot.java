...
public class SNASnapshot extends AbstractSpectrumSnapshot {
    ...
    public void setSNARegisterValues(byte[] sna_header) {
        ...
        int im_iff = ((sna_header[19] & 4) == 0) ? 0 : 3;
    ...
    public void read(InputStream is) {
        ...
        try {
            ...
            LOG.error("Unable to read SNA header.", e);
            System.exit(1);
        }
        ...
        try {
            ...
            LOG.error("Unable to read SNA snapshot memory contents.", e);
            System.exit(1);
        }
    ...
    public void getSNARegisterValues(byte[] sna_header) {
        sna_header[0] = regs[Z80.I];
        sna_header[1] = regs[Z80.L_ALT];
        sna_header[2] = regs[Z80.H_ALT];
        sna_header[3] = regs[Z80.E_ALT];
        sna_header[4] = regs[Z80.D_ALT];
        sna_header[5] = regs[Z80.C_ALT];
        sna_header[6] = regs[Z80.B_ALT];
        sna_header[7] = regs[Z80.F_ALT];
        sna_header[8] = regs[Z80.A_ALT];
        sna_header[9] = regs[Z80.L];
        sna_header[10] = regs[Z80.H];
        sna_header[11] = regs[Z80.E];
        sna_header[12] = regs[Z80.D];
        sna_header[13] = regs[Z80.C];
        sna_header[14] = regs[Z80.B];
        sna_header[15] = regs[Z80.YL];
        sna_header[16] = regs[Z80.YH];
        sna_header[17] = regs[Z80.XL];
        sna_header[18] = regs[Z80.XH];
        int im_iff = regs[Z80.IM_IFF];
        sna_header[19] = (byte) (((im_iff & 2) == 0) ? 0 : 0xff);
        sna_header[20] = regs[Z80.R];
        sna_header[21] = regs[Z80.F];
        sna_header[22] = regs[Z80.A];
        sna_header[23] = regs[Z80.SPL];
        sna_header[24] = regs[Z80.SPH];
        sna_header[25] = (byte) ((im_iff & 0xc) >> 2);
        sna_header[26] = (byte) (border & 7);
    }

    public void write(OutputStream os) {
        if (SwingUtilities.isEventDispatchThread()) {
            LOG.fatal("\n  Attempted to save SNA snapshot from event dispatch thread.\n"
                    + "This is not allowed, because it is a possibly time-consuming task\n"
                    + "and not thread safe with main emulator loop thread.");
            System.exit(1);
        }

        /* Pushes the value of program counter to top of the stack. */
        int sp = (regs[Z80.SPL] & 0xff) | ((regs[Z80.SPH] & 0xff) << 8);
        sp -= 2;
        memory[sp] = regs[Z80.PCL];
        memory[sp + 1] = regs[Z80.PCH];
        regs[Z80.SPL] = (byte) (sp & 0xff);
        regs[Z80.SPH] = (byte) ((sp & 0xff00) >> 8);

        byte[] sna_header = new byte[27];
        getSNARegisterValues(sna_header);
        try {
            os.write(sna_header);
        } catch (Exception e) {
            LOG.error("Unable to write SNA header.", e);
            System.exit(1);
        }
        ...
        try {
            ...
            LOG.error("Unable to write memory contents to SNA snapshot.", e);
            System.exit(1);
        }
    }
}
